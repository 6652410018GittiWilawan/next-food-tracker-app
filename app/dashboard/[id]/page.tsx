    ...
    <div className="min-h-screen bg-amber-50 p-8">
      <div className="container mx-auto max-w-4xl rounded-lg bg-white p-6 shadow-lg">
        <div className="flex justify-between items-center">
          <h1 className="mb-6 text-3xl font-bold text-amber-800">
            Food Dashboard
          </h1>
          {users.map((user) => (
        ...
        <div className="mb-6 flex flex-col items-center justify-between gap-4 md:flex-row">
          <Link href={"/addfood/" + id}>
            <div className="w-full transform rounded-full bg-amber-500 px-6 py-2 font-semibold text-white shadow-md transition-all duration-300 hover:scale-105 hover:bg-amber-600 md:w-auto">
              + Add Food
            </div>
          </Link>
        </div>
                ...
                <tr key={key}>
                  ...
                  <td className="px-6 py-3 text-right">
                    <Link href={`/updatefood/${food.id}`}>
                      <button className="mr-2 transform rounded-full bg-amber-500 px-4 py-2 font-semibold text-white shadow-md transition-all duration-300 hover:scale-105 hover:bg-amber-600">
                        แก้ไข
                      </button>
                    </Link>
                    <button
